...
<script>
    window.addEvent('domready', function() {
        ...
        var selectClient = $('adminForm').getElement('select[name=filter_client_id]');
        // remove active class
        $$('#site, #admin').removeClass('active');
        // set the new active element
        selectClient.get('value') == 0 ? $('site').addClass('active') : $('admin').addClass('active');
        // fire the event on #submenu
        $('submenu').getElements('a').addEvent('click', function() {
            var val = this.get('id') == 'site' ? 0 : 1;
            selectClient.set('value',val);
            Joomla.submitform();
        });
    });
</script>
<ul id="submenu" class="out">
    <li>
        <a class="active" href="#" id="site"><?php echo JText::_('TPL_MINIMA_SITE'); ?></a>
    </li>
    <li>
        <a href="#" id="admin"><?php echo JText::_('TPL_MINIMA_ADMINISTRATOR'); ?></a>
    </li>
</ul>
